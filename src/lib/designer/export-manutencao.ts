...
export function sufixoLimpo(cfg: ManutencaoConfig): string {
  return (
    (cfg.sufixoFuncao || "PlanoManutencao")
      .normalize("NFD")
      .replace(/[̀-ͯ]/g, "")
      .replace(/[^a-zA-Z0-9]+/g, "") || "PlanoManutencao"
  );
}
...
export function nomeFuncao(
  ...
  return prefixo + sufixoLimpo(cfg);
}
...
function fmtNum(v: number, prec: number): string {
...
type Fonte = ReturnType<typeof criarFonte>;

// ─── cabeçalho / rodapé ───────────────────────────────────────────────────────

/** Recorte de dia/mês/ano por posição — o formulário grava a data como string crua. */
function substrsDaData(formato: "ymd" | "dmy", anoDigitos: "2" | "4"): {
  dia: string;
  mes: string;
  ano: string;
} {
...
function headerFieldLines(field: HeaderField, indent: string, fonte: Fonte, resp: string): string[] {
  const out: string[] = [];
